...
class MICD(object):
    ...
    def classify(self, points):
        ...
        R = ((Z[:,:, None] * self.class_invs).sum(axis=1) * Z).sum(axis=1)
...
class KMeans(object):
    ...
    Note: This is potentially unsafe (you could get a means array of NaN),
    This occurs when two of the cluster heads are the same, this is possible
    if more than one of the same pattern are in the training points.
    I've been working around this by just reinitialising if that happens, there
    is currently no np.unique(axis=0) method, and I don't feel like resorting
    to using python's set ops (although that is the defacto std solution).
    Cheers.
    '''
